...
```
.
├── config/              # Suckless utilities configuration headers
│   ├── dmenu/
│   ├── dwm/
│   ├── slstatus/
│   └── st/
...
│   ├── applications/   # Application packages and configurations
│   ├── core/          # Essential system setup (users, boot, services)
│   ├── desktop/       # Window manager configurations
│   └── drivers/       # Hardware driver modules
└── profiles/          # Hardware profiles (presets for driver selection)
    ├── amd/
    ├── intel/
    ├── nvidia/
    ├── nvidia-laptop/
...
- **AMD GPU** → `profile = "amd";`
- **Intel GPU** → `profile = "intel";`
- **NVIDIA Desktop** → `profile = "nvidia";`
- **NVIDIA Laptop** → `profile = "nvidia-laptop";`
- **Virtual Machine** → `profile = "vm";`
...
## 🛠️ Example: Default Configuration
